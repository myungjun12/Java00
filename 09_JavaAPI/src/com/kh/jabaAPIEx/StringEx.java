...
/*
 ...
 String str = "";
  ...
  subString = 내가 시작하고 싶은 숫자부터 끝내고 싶은 숫자 값을 지정해서 변경할 수 있음
  subString(시작숫자,끝나는숫자) = 시작숫자와 ~ 끝나는숫자 사이에 값 출력
  subString(시작숫자) // 시작숫자이후로 모두 출력
  toUpperCase() = 원하는 문자열을 모두 대문자로 변환
  toLowerCase() = 원하는 문자열을 모두 소문자로 변환
  		 trim() = 원하는 문자열의 앞 뒤 공백을 모두 제거해서 return(반환) 
...
public class StringEx {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 String 문자열 = "H e l l o w W o r l d !";  
		 * */
		...
		System.out.println("대문자 맞나요 ? ");
		System.out.println(대문자);
		// 3. toLowerCase()
		String 소문자 = 문자열.toLowerCase();
		System.out.println("소문자 맞나요 ? ");
		System.out.println(소문자);
		// 4. trim()
		String 공백가득 = "    Hello    World!      "; 
		String 공백제거 = 공백가득.trim(); //중간 공백은 제거되지 않지만 양쪽 사이드 공백이 제거된다.
		System.out.println("공백제거 됬나요 ?");
		System.out.println(공백제거);
		// split splice replace match charAt 외우자
