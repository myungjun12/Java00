...
메서드(Method)
	...
	생성자 : 파일명, 클래스이름과 똑같이 작성을 해줘야하고 객체의 기본 값을 설정하거나 값을 각각 변경해서 사용하는데 쓰이는 메서드
		기본 : 매개변수가 필수로 없어도 되는 생성자 / 필수 입력이 없음
		public 클래스이름(){
			
		}
		필수 : 매개변수가 필수로 존재해야하는 생성자 / 필수로 작성해줬으면 하는 값을 표기
		public 클래스이름(자료형 매개변수,자료형 매개변수){
			this.변수명 = 매개변수;
			this.변수명 = 매개변수;
		}
			예를들어 회원가입을 진행할 때 필수로 이름 나이 () 넣어주고
				결혼기념일 광고수신동의는 선택으로 하고 싶다면 필수로 이름 ~ 주소를 넣어줌
			public 클래스이름 (String name, int age) {
				this.name = name;
				this.age = age;
			}
			...
			return : 어떤 행동을 실행하고 행동을 한 다음에 행동에 대한 결과를 필요한 곳에 전달
				...
				//문자열을 전달 = String
				 public String 특정행동1() {
				 	return "안녕하세요"; // 위에 String을 작성했으니 return 다음에 String 값을 작성하고 반환한다
				 }
				 
				 public int 특정해동2() {
				 	return 1; // 반환타입 int임으로 리턴타입도 int로 반환 
				}
				
				main : 최종으로 실행하거나 최종으로 실행하고 싶은 내용만 작성하는 공간
