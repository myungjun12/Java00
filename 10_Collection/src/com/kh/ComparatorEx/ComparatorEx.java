...
reversed() : 순서를 거꾸로 뒤집어서 순서를 매길 때 사용
	reversed : 반전
comparing(클래스명 :: 참조할메서드명) 클래스 밑에서 참조할 메서드를 가지고와서 정렬 기준 선정
 * */
public class ComparatorEx {
	public static void main(String[] args) {
		List<String> 단어들 = new ArrayList<>(Arrays.asList("apple","banana","cherry"));
		
		//단어에서 단어의 길이를 기준으로 정렬하자			클래명 :: 참조할메서드명
		Comparator<String> 길이비교 = Comparator.comparing(String::length);
				//String 클래스에서 밑에 있는 메서드 중에서 길이를 나타내는 length메서드를 사용해서 비교
		
		//sort 사용해서 정렬
		단어들.sort(길이비교);
		
		//정렬된 리스트 출력
		System.out.println("문자열 길이 비교 : " + 단어들);
	}
}
